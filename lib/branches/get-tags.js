    ...
    async (branches, branch) => {
      const branchTags = await pReduce(
        ...
        async (branchTags, tag) => {
          // In order to support composer pre-release versions that contain a "dev-" prefix, we need to remove it for
          // the version checking. We do need to keep the original tag name for the git reference though.
          let rawVersionTag = tag
          if (tag && tag.includes("dev-")) {
            rawVersionTag = tag.replace("dev-", "");
          }

          const [, version] = rawVersionTag.match(tagRegexp) || [];
